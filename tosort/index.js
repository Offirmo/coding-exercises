    ...
    request(restUrl, (error, response, body) => {
        ...
        if (!error && response.statusCode == 200) {
            ...
            if (onboarding && onboarding.solution) {
                console.log('there is a solution, but...')
                //tpl = onboarding.solution;
            }

            let track = model.track;

            let subDir = path.join('domains', track.track_slug, track.slug, model.slug);
            fs.writeFileSync('.current', subDir);
            let dir = path.join(process.cwd(), subDir);
            ...
            let soluceFile = path.join(dir, 'soluce_' + engine.main);
            ...
            try {
                ...
                fs.writeFileSync(file, tpl, 'binary');
            ...
            if (onboarding && onboarding.solution) {
                ...
                fs.writeFileSync(soluceFile, onboarding.solution, 'binary');
            }
        }
    });
